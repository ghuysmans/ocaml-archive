...
module Read = struct
  ...
  let with_low_fn t fn f =
    try
      let path = ArchiveLow.Entry.pathname (MapString.find fn t.ard_tree) in
      with_low_arch t.ard_input (fun hdl ->
          let ent = ArchiveLow.Entry.create () in

          let rec find () =
            if ArchiveLow.Read.next_header2 hdl ent then
              if ArchiveLow.Entry.pathname ent = path then f hdl ent
              else find ()
            else raise Not_found
          in
          find ())
    with Not_found -> raise (Unix_error (ENOENT, "open", fn))

  let create input =
    with_low_arch input (fun hdl ->
        let ent = ArchiveLow.Entry.create () in
        let rec fold acc =
          if ArchiveLow.Read.next_header2 hdl ent then
            fold
        ...
        in
        { ard_input = input; ard_tree = fold MapString.empty })

  let file_exists t fn = MapString.mem fn t.ard_tree

  let stat t fn =
    try ArchiveLow.Entry.stat (MapString.find fn t.ard_tree)
    with Not_found -> raise (Unix_error (ENOENT, "stat", fn))

  let is_directory t fn = (stat t fn).st_kind = S_DIR

  let with_file t fn finit fread fclose =
    with_low_fn t fn (fun hdl ent ->
  ...
  let entry t fn = MapString.find fn t.ard_tree

  let content t fn =
    with_file t fn
      (fun ent ->
        Buffer.create (Int64.to_int (ArchiveLow.Entry.stat ent).st_size))
      (fun buf str len ->
    ...
    let lst =
      MapString.fold
        (fun fn' _ acc ->
          ...
          if
            (FilePath.is_current fn && FilePath.is_current dn')
            || FilePath.compare dn' fn = 0
          then FilePath.basename fn' :: acc
          else acc)
        t.ard_tree []
    in
    Array.of_list lst
