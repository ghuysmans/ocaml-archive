...
module Read = struct
  type 'ctx t = ([ `Read ], 'ctx) archive

  external create : unit -> _ t = "caml_archive_read_create"
  (** archive_read_new *)

  external support_filter_all : _ t -> unit
    = "caml_archive_read_support_filter_all"
  (** archive_read_support_filter_all *)

  external support_format_all : _ t -> unit
    = "caml_archive_read_support_format_all"
  (** archive_read_support_compress_all *)

  external open_filename : _ t -> filename -> int -> unit
    = "caml_archive_read_open_filename"
  (** archive_read_open_filename *)

  external set_seek_callback : 'a t -> 'a seek_callback -> unit
  ...
  external next_header2 : _ t -> Entry.t -> bool
    = "caml_archive_read_next_header2"

  external data_skip : _ t -> unit = "caml_archive_read_data_skip"
  external data : _ t -> string -> int -> int -> int = "caml_archive_read_data"
  external close : _ t -> unit = "caml_archive_read_close"
end
